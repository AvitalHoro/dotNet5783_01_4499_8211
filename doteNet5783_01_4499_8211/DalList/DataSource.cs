    ...
    //creates list of new orders
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            ListOrder.Add(new Order
            {
                ...
                CostumerEmail = costumerEmail[randNum.Next(costumerEmail.Length)], //מגריל אימייל של לקוח רנדומלי מהמערך של האימיילים
                CostumerAdress = costumerAdress[randNum.Next(costumerAdress.Length)],//מגריל כתובת של לקוח רנדומלי מהמערך של הכתובות
                OrderDate = date + new TimeSpan(randNum.NextInt64(10L * 1000L * 3600L * 24L * 8000L, 10L * 1000L * 3600L * 24L * 10000L)),
                //לוקח את התאריך של היום, מוריד ממנו זמן מסוים, וכך מתקבל זמן הזמנה הגיוני
                DeliveryDate =null, 
                ShipDate=null,
                IsDeleted = false,
        ...
        for (int i = 0; i < 8; i++)
        {
            ListOrder.Add(new Order
            {
                ...
                CostumerEmail = costumerEmail[randNum.Next(costumerEmail.Length)],//מגריל אימייל של לקוח רנדומלי מהמערך של האימיילים
                CostumerAdress = costumerAdress[randNum.Next(costumerAdress.Length)],//מגריל כתובת של לקוח רנדומלי מהמערך של הכתובות
                OrderDate = date + new TimeSpan(randNum.NextInt64(10L * 1000L * 3600L * 24L * 4000L , 10L * 1000L * 3600L * 24L * 6000L)),
                //לוקח את התאריך של היום, מוריד ממנו זמן מסוים, וכך מתקבל זמן הזמנה הגיוני
                ShipDate = date + new TimeSpan(randNum.NextInt64(10L * 1000L * 3600L * 24L * 6000L, 10L * 1000L * 3600L * 24L * 10000L)),
                //לוקח את התאריך של היום, מוריד ממנו זמן מסוים, וכך מתקבל זמן שליחה הגיוני
                DeliveryDate = null,
                IsDeleted = false,
            });
